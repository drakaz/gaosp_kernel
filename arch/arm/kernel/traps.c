...
extern void *MSM_KERNEL_PANIC_DUMP_ADDR;
void *PANIC_Base;
void *PANIC_Current;
extern int multiple_panic;
...
NORET_TYPE void die(const char *str, struct pt_regs *regs, int err)
{
...
#ifdef CONFIG_KERNEL_PANIC_DUMP
	if(multiple_panic) {
	    printk("## Protect Multiple Panic\n");
	    panic("Multiple Panic");
	}

	kernel_panic_logging = 1;
	PANIC_Base = (void *)MSM_KERNEL_PANIC_DUMP_ADDR;
	if (!PANIC_Base)
    {
	...
	PANIC_Current += sprintf(PANIC_Current,"                                   GA LAB.\n");
	PANIC_Current += sprintf(PANIC_Current,"---------------------------------------------\n");
	...
	PANIC_Current += sprintf(PANIC_Current,"        contact us : android@samsung.com\n");
	PANIC_Current += sprintf(PANIC_Current,"---------------------------------------------\n\n");
#endif
	oops_enter();
