...
static int smd_rpcrouter_debug_mask = SMEM_LOG; //added for SMEM log
...
struct msm_rpc_endpoint *msm_rpcrouter_create_local_endpoint(dev_t dev)
{
	...
	wake_lock_init(&ept->read_q_wake_lock, WAKE_LOCK_SUSPEND, "rpc_read");  // mod_0707
...
int msm_rpcrouter_destroy_local_endpoint(struct msm_rpc_endpoint *ept)
{
	...
	wake_lock_destroy(&ept->read_q_wake_lock);  // mod_0707
	list_del(&ept->list);
	kfree(ept);
	return 0;
}
...
packet_complete:
	...
	wake_lock(&ept->read_q_wake_lock);  // mod_0707
		   ...
		   unsigned len, long timeout)
{
	...
	list_del(&pkt->list);
	if (list_empty(&ept->read_q))    // mod_0707
		wake_unlock(&ept->read_q_wake_lock);
	spin_unlock_irqrestore(&ept->read_q_lock, flags); // mod_0707
