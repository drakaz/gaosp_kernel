...
void reset_and_upload_mode(void);
int sleep_fail_count = 0;
	...
	if (enter_state) {
		...
		*msm_pm_sma.limit_sleep = sleep_limit;
		...
		if (ret) {
			...
			sleep_fail_count ++;
			/*
			if( sleep_fail_count > 0 )
				while(1) reset_and_upload_mode();
			*/

			//restart_reason = 0x776655AA;
			/* silent reset */
			restart_reason = 0xAA556677;
...
void reset_and_upload_mode(void)
{
	u16 reboot_magic_code = 0x3569;
	SmemBase = (volatile unsigned char *)(smem_alloc(SMEM_ID_VENDOR0, 0x4000*2));
	WRITE_TO_DPRAM(DPRAM_MAGIC_CODE_ADDRESS, &reboot_magic_code, sizeof(reboot_magic_code));
	smsm_change_state(SMSM_APPS_STATE, SMSM_RESET | SMSM_SYSTEM_DOWNLOAD, SMSM_RESET | SMSM_SYSTEM_DOWNLOAD);
}
// }}
