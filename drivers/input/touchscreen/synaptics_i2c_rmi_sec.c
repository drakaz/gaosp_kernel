...
//	printk("[KHG] synaptics_ts_work_func\n");  
	ret = i2c_transfer(ts->client->adapter, msg, 2);
	if (ret < 0) {
		printk("synaptics_ts_work_func: i2c_transfer failed\n");
	} else {
		  // printk("synaptics_ts_work_func: %x %x %x %x %x %x %x %x\n", 
        //			      buf[0], buf[1], buf[2], buf[3],  
		//       buf[4], buf[5], buf[6], buf[7]); 
			int x = buf[3] | (uint16_t)(buf[2] & 0x1f) << 8;
			int y = buf[5] | (uint16_t)(buf[4] & 0x1f) << 8;
#if 0
			int real_x = (int)((float)(x * (320/3619)));
			int real_y = (int)((float)(y * (480/5418)));	
#else
			int real_x = (x * 320)/3619;
			int real_y = (y * 480)/5418;	
#endif
			int z = buf[1];
			int w = buf[0] >> 4;
			//int finger = buf[0] & 7;
			int finger = buf[0] & 0x01;
			int dx = (int8_t)buf[6];
			int dy = (int8_t)buf[7];

			//  printk("x %4d, y %4d, z %3d, w %2d, F %d, dx %4d, dy %4d\n", 
			//	real_x, 480-real_y, z, w, finger,dx, dy); 
//			if (x)
				input_report_abs(ts->input_dev, ABS_X, real_x);
//			if (y)
//				input_report_abs(ts->input_dev, ABS_Y, ts->max_y + 1 - y);
			input_report_abs(ts->input_dev, ABS_Y, 480-real_y);		
...
MODULE_DESCRIPTION("Synaptics Touchscreen Samsung Driver");
MODULE_LICENSE("GPL");
