...
static int dpram_thread(void *data)
{
	...
	while (1) {
		...
		if (ret == -ERESTARTSYS) {
			...
			/* demux ?�패?�더?�도 ?�로 메시지 출력?��? ?�는??
			 * 출력?�는 메시지 ?�문????parsing???�려지???�상 발생. */
			char ch;
			dpram_read(dpram_filp, &ch, sizeof(ch));

			/* start byte가 ?�치???�만 demuxing ?�어간다. */
			if (ch == 0x7f) {
				pdp_demux();
			}
		}
...
static int vs_read(struct pdp_info *dev, size_t len)
{
	...
	if(!dev->vs_dev.refcount)
		printk("[%s] failed.. this tty channel(Id:%d) is not opened.\n", __func__, dev->id);
