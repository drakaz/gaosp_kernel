...
#if 1  // Exports functions for rfkill1 driver
int msm_hs_uart_power_ex(int on)
{
	struct msm_hs_port *msm_uport;
	struct uart_port *uport;
				 
	msm_uport = &q_uart_port[0];  // ttyHS0
	uport = (struct uart_port *)&msm_uport->uport;

	if (!msm_uport) {
		printk(KERN_ERR  "%s: [err] msm_uport is null!\n", __func__, uport);
		return -1;
	}

	//printk( "%s: hs_uart clk %s (uport=0x%x)\n" , __func__, on?"ON":"OFF", uport);
	if (on) {
		#if 1
		...
		#if 1
		msm_hs_set_mctrl_locked(uport, 0);
...
static struct platform_driver msm_serial_hs_platform_driver = {
	...
	//CASE 00151959 : enter ARM11 power collapse, not TCXO Shutdown
