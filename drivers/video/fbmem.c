...
// for oled on/off test 
extern void test_lcd_on(void);
extern void test_lcd_off(void);
	 ...
	 unsigned long arg)
{
	...
	// for oled on/off test
	case FBIOTEST_LCDOFF:
//		printk("[HSIL TEST] %s : ioctl : LCD OFF\n", __func__);
		test_lcd_off();
		return 0;
	case FBIOTEST_LCDON:
//		printk("[HSIL TEST] %s : ioctl : LCD ON\n", __func__);
		test_lcd_on();
		return 0;
