...
struct gs_dev {
	...
	struct usb_interface_descriptor gs_ifc_desc2;
...
static void gs_init_ifc_desc2(struct usb_interface_descriptor *ifc_desc)
{
	ifc_desc->bLength =		USB_DT_INTERFACE_SIZE;
	ifc_desc->bDescriptorType =	USB_DT_INTERFACE;
...
static void gs_init_header_desc(struct gs_dev *dev)
{
	...
	dev->gs_highspeed_header[6] = 
		(struct usb_descriptor_header *)&dev->gs_ifc_desc2;
...
static int __init gs_module_init(void)
{
	...
	for (i = 0; i < instances; i++) {
		...
		/* 2 - Interface, 3 Endpoints, 4 CDC/ACM spec header +1 for NULL */
		...
		gs_init_ifc_desc2(&gs_devices[i]->gs_ifc_desc2);
...
static void gs_bind(void *_ctxt)
{
	...
	ret = usb_msm_get_next_ifc_number(func);
	dev->gs_ifc_desc2.bInterfaceNumber = ret;
	dev->gs_ifc_desc2.iInterface = 0;
	...
	pr_debug("%s: bulk_in_endpoint Number = %d\n",
						__func__, ep->num);
//	printk("%s: bulk_in_endpoint Number = %d\n",__FUNCTION__,ep->num);
	...
	pr_debug("%s: bulk_out_endpoint Number = %d\n",
						__func__, ep->num);
//	printk("%s: bulk_out_endpoint Number = %d\n",__FUNCTION__,ep->num);
	...
	pr_debug("%s: notify_in_endpoint Number = %d\n",
						__func__, ep->num);
//	printk("%s: notify_in_endpoint Number = %d\n",__FUNCTION__,ep->num);
		...
		void *buf, int len, void *_ctxt)
{
	...
	case USB_CDC_REQ_SET_LINE_CODING:
		...
		if (port) {
			printk("before port lock\n");
			spin_lock(&port->port_lock);
			memcpy(&port->port_line_coding, buf , ret);
			spin_unlock(&port->port_lock);
			printk("after port lock\n");
			ret = 0;
		}
		ret = wLength-8;
#endif
