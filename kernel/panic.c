...
/* kernel panic dump code added by gtuo.park in SD */
...
NORET_TYPE void panic(const char * fmt, ...)
{
	...
	if(!multiple_panic) {
		multiple_panic++;
		id =  current->files;
		printk("panic data write to /data/panic \n");
		
		preempt_enable();

		old_fs = get_fs();
		set_fs(KERNEL_DS);
		printk("set fs to KERNEL_DS\n");

		count = PANIC_Current - PANIC_Base;
		chr_count = 0;
		filp = filp_open("/data/panic",O_CREAT|O_WRONLY,0666);
		if(IS_ERR(filp))
			printk("Can't creat /data/panic file\n");
		else {
			printk("panic data write start.\n");
			while(count) {
				memset(buf,0x0,1024);
				if(count > 1024) {
