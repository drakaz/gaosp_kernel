...
extern struct snd_soc_dai msm7201_dai;
extern struct snd_soc_dai msm_cpudai;
...
static int msm_soc_probe(struct platform_device *devptr)
{
...
static int msm_soc_remove(struct platform_device *devptr)
{
	struct snd_soc_device *socdev = platform_get_drvdata(devptr);
	snd_soc_free_pcms(socdev);
	kfree(socdev->codec);
...
static int __init msm_audio_init(void)
{
	...
	ret = snd_soc_register_platform(&msm_soc_platform);
	if (ret ) {
		printk("Can not register soc platform\n");
		return ret;
	}
